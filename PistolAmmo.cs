...
public class PistolAmmo : MonoBehaviour
{
    ...
    void Start ()
    {
        ...
        numberOfRoundsInBox = Random.Range(4, 10);
    ...
    **************************************************************************/
    private void PickUpItem()
    {
        ...
        if (itemManager.CheckForEmptyPanel()) //checks if there's an empty item slot
        {
            ...
            if (itemManager.CheckForOccupiedPanel(gameObject.tag))
            {
                ...
                else
                {
                    ...
                    itemManager.AddToPistolAmmoCount(numberOfRoundsInBox - excessAmmo);
                      //store index of empty item slot
                    itemManager.CheckForEmptyPanel();

                    if (excessAmmo <= itemManager.GetMaxPanelPistolAmmo())
                    {
                        ...
                        itemManager.AddToPistolAmmoCount(excessAmmo);
                        numberOfRoundsInBox = 0;
                        excessAmmo = 0;
                        Destroy(gameObject);
                    }
            ...
            else if (numberOfRoundsInBox > itemManager.GetMaxPanelShotgunAmmo())
            {
                ...
                excessAmmo = (numberOfRoundsInBox - itemManager.GetMaxPanelPistolAmmo());
        ...
        else if (itemManager.CheckForOccupiedPanel(gameObject.tag))
        {
            ...
            if (itemManager.GetPanelItemCount() + numberOfRoundsInBox <= itemManager.GetMaxPanelPistolAmmo())
            {
                itemManager.AddToItemPanel(gameObject.tag);
                itemManager.AddToPistolAmmoCount(numberOfRoundsInBox);
                numberOfRoundsInBox = 0;
