...
public class LargeFirstAidKit : MonoBehaviour
{
    ...
    **************************************************************************/
    private void HitByRay()
    {
        hudManager.DisplayPrompt(); //displays prompt to pick up item
          //checks if player presses E while game isn't paused
    ...
    **************************************************************************/
    private void PickUpItem()
    {
        ...
        if (itemManager.CheckForEmptyPanel()) //checks if there's an empty item slot
        {
            ...
            if (itemManager.CheckForOccupiedPanel(gameObject.tag))
            {
                ...
                else
                {
                    ...
                    itemManager.AddToLargeFirstAidCount(numberOfFirstAidKits - excessKits);
                      //store index of empty item slot
                    itemManager.CheckForEmptyPanel();

                      //checks if excess fits in a single item slot
                    if (excessKits <= itemManager.GetMaxPanelLargeAidKit())
                    {
                        ...
                        itemManager.AddToLargeFirstAidCount(excessKits);
                        numberOfFirstAidKits = 0;
                        excessKits = 0;
                        Destroy(gameObject);
                    }
            ...
            else if (numberOfFirstAidKits > itemManager.GetMaxPanelLargeAidKit())
            {
                  //calculates excess kits
                excessKits = (numberOfFirstAidKits - itemManager.GetMaxPanelLargeAidKit());
            ...
            hudManager.DisplayPickUpText(gameObject.tag); //displays text of large aid kit
            hudManager.DisplayImage(gameObject.tag); //displays image of large aid kit
        }
        ...
        else if (itemManager.CheckForOccupiedPanel(gameObject.tag))
        {
            ...
            if (itemManager.GetPanelItemCount() + numberOfFirstAidKits <= itemManager.GetMaxPanelLargeAidKit())
            {
