/******************************************************************************
  File Name: EnemyType2.cs
  Author(s): Kyle Powell (kyle.powell)
    Project: Dead Letter - Prototype 2.0

Description: This file contains functions that manipulate the second type of 
...
public class EnemyType2 : MonoBehaviour
{
    ...
    [SerializeField] private WeaknessType2 weaknessType2 = null;
    ...
    private NavMeshAgent agent; //used for AI pathing
    ...
    private Quaternion rotationTowardsNode; //used to rotate towards node
    ...
    //TODO: tweak the number for type2 enemies later
...
Description: This function checks to see if the player is within range to
    ...
    **************************************************************************/
    private void RevealWeakPoint()
    {
        ...
        //checks if the player is within 8 meters of the enemy
        if (Vector3.Distance(transform.position, firstPersonController.transform.position) <= upgradeManager.GetWeakness2Range())
        {
            weaknessType2.gameObject.SetActive(true);
        }
        else
        {
            weaknessType2.gameObject.SetActive(false);
        }
    }
...
Description: This function calculates a vector towards the default location and
    ...
    **************************************************************************/
    ...
    {
          //normalizes vector from enemy position to node
        newDirection = (node.transform.position - transform.position).normalized;
          //converts vector to quaternion
        rotationTowardsNode = Quaternion.LookRotation(newDirection);
          //rotates enemy towards node
        transform.rotation = Quaternion.Slerp(transform.rotation, rotationTowardsNode, Time.deltaTime * 3.0f);
    }
...
Description: This function causes the enemy to navigate its way back to its
    ...
    **************************************************************************/
    private void MoveToOrigin()
    {
        ...
        //TODO: include jumpable obstacles for smarter enemies
    }
...
Description: This function determines how many items to spawn in addition to a
    ...
    **************************************************************************/
    private void SpawnLoot()
    {
        ...
        int numberOfSpawnsChance = Random.Range(1, 500);
