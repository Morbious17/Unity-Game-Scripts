/******************************************************************************
  File Name: SupportShotgunAmmo.cs
...
public class SupportShotgunAmmo : MonoBehaviour
{
    ...
    private int excessAmmo; //number of shells that don't fit in current item slot
    ...
    void Start()
    {
        ...
        numberOfRoundsInBox = Random.Range(4, 12);
    ...
    private void PickUpItem()
    {
        ...
        if (itemManager.CheckForEmptyPanel()) //checks if there's an empty item slot
        {
            ...
            if (itemManager.CheckForOccupiedPanel(gameObject.tag))
            {
                ...
                else
                {
                    ...
                    itemManager.AddToSupportShotgunAmmoCount(numberOfRoundsInBox - excessAmmo);
                      //store index of empty item slot
                    itemManager.CheckForEmptyPanel();

                      //checks if excess ammo in a single item slot
                    if (excessAmmo <= itemManager.GetMaxSupportPanelShotgunAmmo())
                    {
                        ...
                        itemManager.AddToSupportShotgunAmmoCount(excessAmmo);
                        numberOfRoundsInBox = 0;
                        excessAmmo = 0;
                        Destroy(gameObject);
                    }

                    numberOfRoundsInBox = excessAmmo; //sets remaining number of shells
                    excessAmmo = 0; //clears excess variable
                }
            }
              //checks if number of shells doesn't fit in a single empty slot
            else if (numberOfRoundsInBox > itemManager.GetMaxSupportPanelShotgunAmmo())
            {
                  //calculates excess shells
                excessAmmo = (numberOfRoundsInBox - itemManager.GetMaxSupportPanelShotgunAmmo());
          ...
          //checks if there's an item slot with room for more shotgun rounds
        else if (itemManager.CheckForOccupiedPanel(gameObject.tag))
        {
            ...
            if (itemManager.GetPanelItemCount() + numberOfRoundsInBox <=
                itemManager.GetMaxSupportPanelShotgunAmmo())
            {
                  //add shells to partially filled panel
                itemManager.AddToItemPanel(gameObject.tag);
                  //add shells to total count
                itemManager.AddToSupportShotgunAmmoCount(numberOfRoundsInBox);
                numberOfRoundsInBox = 0;
