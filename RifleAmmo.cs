...
public class RifleAmmo : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        numberOfRoundsInBox = Random.Range(2, 4);
        ...
        if (itemManager.CheckForEmptyPanel()) //checks if there's an empty item slot
        {
            ...
            if (itemManager.CheckForOccupiedPanel(gameObject.tag))
            {
                ...
                else
                {
                    ...
                    itemManager.AddToRifleAmmoCount(numberOfRoundsInBox - excessAmmo);
                    ...
                    if (excessAmmo <= itemManager.GetMaxPanelRifleAmmo())
                    {
                        ...
                        itemManager.AddToRifleAmmoCount(excessAmmo);
                        numberOfRoundsInBox = 0;
                        excessAmmo = 0;
                        Destroy(gameObject);
                    }
            ...
            else if (numberOfRoundsInBox > itemManager.GetMaxPanelRifleAmmo())
            {
                  //calculates excess shells
                excessAmmo = (numberOfRoundsInBox - itemManager.GetMaxPanelRifleAmmo());
