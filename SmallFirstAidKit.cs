...
public class SmallFirstAidKit : MonoBehaviour
{
        ...
        if (itemManager.CheckForEmptyPanel()) //checks if there's an empty item slot
        {
            ...
            if (itemManager.CheckForOccupiedPanel(gameObject.tag))
            {
                ...
                else
                {
                    ...
                    itemManager.AddToSmallFirstAidCount(numberOfFirstAidKits - excessKits);
                      //store index of empty item slot
                    itemManager.CheckForEmptyPanel();

                      //checks if excess fits in a single item slot
                    if (excessKits <= itemManager.GetMaxPanelSmallAidKit())
                    {
                        ...
                        itemManager.AddToSmallFirstAidCount(excessKits);
                        numberOfFirstAidKits = 0;
                        excessKits = 0;
                        Destroy(gameObject);
                    }
            ...
            else if (numberOfFirstAidKits > itemManager.GetMaxPanelSmallAidKit())
            {
                  //calculates excess kits
                excessKits = (numberOfFirstAidKits - itemManager.GetMaxPanelSmallAidKit());
