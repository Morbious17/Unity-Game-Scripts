...
public class ShotgunAmmo : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        numberOfRoundsInBox = Random.Range(2, 6);
        ...
        if (itemManager.CheckForEmptyPanel()) //checks if there's an empty item slot
        {
            ...
            if (itemManager.CheckForOccupiedPanel(gameObject.tag))
            {
                ...
                else
                {
                    ...
                    itemManager.AddToShotgunAmmoCount(numberOfRoundsInBox - excessAmmo);
                      //store index of empty item slot
                    itemManager.CheckForEmptyPanel();

                      //checks if excess ammo in a single item slot
                    if (excessAmmo <= itemManager.GetMaxPanelShotgunAmmo())
                    {
                        ...
                        itemManager.AddToShotgunAmmoCount(excessAmmo);
                        numberOfRoundsInBox = 0;
                        excessAmmo = 0;
                        Destroy(gameObject);
                    }
            ...
            else if (numberOfRoundsInBox > itemManager.GetMaxPanelShotgunAmmo())
            {
                ...
                excessAmmo = (numberOfRoundsInBox - itemManager.GetMaxPanelShotgunAmmo());
