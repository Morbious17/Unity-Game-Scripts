/******************************************************************************
  File Name: SupportPistolAmmo.cs
...
public class SupportPistolAmmo : MonoBehaviour
{
    ...
    private int excessAmmo; //stores rounds that won't fit in partially filled item slot
    ...
    void Start()
    {
        ...
        numberOfRoundsInBox = Random.Range(8, 20);
    ...
    **************************************************************************/
    private void PickUpItem()
    {
        ...
        if (itemManager.CheckForEmptyPanel()) //checks if there's an empty item slot
        {
            ...
            if (itemManager.CheckForOccupiedPanel(gameObject.tag))
            {
                ...
                else
                {
                    ...
                    itemManager.AddToSupportPistolAmmoCount(numberOfRoundsInBox - excessAmmo);
                      //store index of empty item slot
                    itemManager.CheckForEmptyPanel();

                    if (excessAmmo <= itemManager.GetMaxSupportPanelPistolAmmo())
                    {
                        ...
                        itemManager.AddToSupportPistolAmmoCount(excessAmmo);
                        numberOfRoundsInBox = 0;
                        excessAmmo = 0;
                        Destroy(gameObject);
                    }
            ...
            else if (numberOfRoundsInBox > itemManager.GetMaxPanelShotgunAmmo())
            {
                  //calculates excess shells
                excessAmmo = (numberOfRoundsInBox - itemManager.GetMaxSupportPanelPistolAmmo());
          ...
          //checks if there's an item slot with room for more pistol rounds
        else if (itemManager.CheckForOccupiedPanel(gameObject.tag))
        {
          ...
          //number should never go below 0, but this is just in case
